...
private:
    DictValue parres;
protected:
    GenericMethod(GenericMsg&& k): ConcretizeMsg<msg_type>(std::move(k)), parres(ConcretizeMsg<msg_type>::get_msg_root()) {
    }
    GenericMethod(const std::string& method_name): ConcretizeMsg<msg_type>(), parres(ConcretizeMsg<msg_type>::get_msg_root()) {
...
public:
    ...
    Derived& add_value(const std::string &name, value_t value) {
...
//    Derived& add_value(const std::string &name, const char* value) {
//        xmlNodePtr msg = ConcretizeMsg<msg_type>::get_msg_root();
...
//    }
//
//    Derived& add_value(const std::string &name, const std::string& value) {
