...
SendFrameMsg::SendFrameMsg(GenericMsg&& k): ConcretizeMsg(std::move(k)) {
}

SendFrameMsg::SendFrameMsg(const H9frame& frame): ConcretizeMsg() {
    ...
    char str[str_sizie];
    std::snprintf(str, str_sizie, "%u", (unsigned int)frame.type);
...
H9frame SendFrameMsg::get_frame() {
    xmlNodePtr root = get_msg_root();
