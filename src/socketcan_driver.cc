...
int SocketCANDriver::recv_data(H9frame* frame) {
    ...
    res.type = H9frame::from_underlying<H9frame::Type>((can_msg.can_id >> (H9frame::H9FRAME_SEQNUM_BIT_LENGTH + H9frame::H9FRAME_DESTINATION_ID_BIT_LENGTH + H9frame::H9FRAME_SOURCE_ID_BIT_LENGTH)) & ((1 << H9frame::H9FRAME_TYPE_BIT_LENGTH) - 1));

    res.seqnum = static_cast<std::uint8_t>((can_msg.can_id >> (H9frame::H9FRAME_DESTINATION_ID_BIT_LENGTH + H9frame::H9FRAME_SOURCE_ID_BIT_LENGTH)) & ((1 << H9frame::H9FRAME_SEQNUM_BIT_LENGTH) - 1));
